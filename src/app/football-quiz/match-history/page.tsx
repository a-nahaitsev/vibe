...
import { MATCH_HISTORY_QUESTIONS } from "../_lib/mock-data";
...
export default function MatchHistoryPage() {
  ...
  const q = MATCH_HISTORY_QUESTIONS[index];
  ...
  const handleNext = useCallback(() => {
    if (index + 1 >= MATCH_HISTORY_QUESTIONS.length) {
      setFinished(true);
      return;
    }
    setIndex((i) => i + 1);
      ...
      <div className="space-y-6">
        ...
        <div className="rounded-xl border border-emerald-200 bg-emerald-50 p-6 dark:border-emerald-800 dark:bg-emerald-900/20">
          ...
          <p className="mt-2 text-zinc-700 dark:text-zinc-300">
            Score: <strong>{score}</strong> / {MATCH_HISTORY_QUESTIONS.length}
          </p>
        </div>
      </div>
    );
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <span className="text-sm text-zinc-600 dark:text-zinc-400">
          {index + 1} / {MATCH_HISTORY_QUESTIONS.length} · Score: {score}
        </span>
      </div>
      <QuestionCard
        question={q}
        questionNumber={index + 1}
        totalQuestions={MATCH_HISTORY_QUESTIONS.length}
      >
        {q.matchContext && (
        ...
        {showResult && (
          ...
          >
            {index + 1 >= MATCH_HISTORY_QUESTIONS.length ? "See results" : "Next"}
          </button>
        )}
      </QuestionCard>
    </div>
